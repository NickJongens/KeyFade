...
exports.retrieveSecret = async (req, res) => {
  ...
  // Log URL params to ensure they are being extracted properly
  ...
  try {
    ...
    const keySecret = await azureService.getSecretFromVault(`${id}-key`);
    ...
    const secret = await azureService.getSecretFromVault(id);
    ...
    // Ensure we are sending the response correctly
    logger.info('Secret retrieved successfully');
    return res.status(200).json({
      name: id,    // Use the ID for the name field in the response
      value: secret // This should be the secret's value
    });
